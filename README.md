...
- m68k -- `restart_syscall-p`, `prctl-seccomp-filter-v`, `seccomp-filter-v`: on
  m68k libc reads the thread pointer via `get_thread_area`, which polluted
  attach traces and tripped the seccomp filters.
- m68k -- `net-sockaddr`: `struct sockaddr_ax25` is 14 bytes on m68k (2-byte
  `int` alignment) vs 16 elsewhere.
- sparc64 -- `mmap`: the hardcoded mmap hint address the test passes fell inside
  the sparc64 VA hole.
